...
- [ ] Vite + TS + React, базовая структура, ESLint/Prettier.
- [ ] Состояние игры (`Game`, `World`, `CityState`), ивенты.
- [ ] Рендер сетки 20×20, выбор клетки, размещение `Road`.
- [ ] Панель инструментов (выбор типа постройки), undo/redo (простой стек).

### Этап B — Здания и экономика (2–3 дня)
- [ ] Типы зданий: `Residential`, `Industrial`, `PowerPlant`.
...
- [ ] Подсчёт `supply/demand`, `coverage`.
- [ ] Влияние на производительность промышленности и удовлетворённость.
- [ ] Оверлей Energy (градиенты/иконки отключения).

### Этап D — Популяция (2 дня)
- [ ] Вместимость жилых, рабочие места, занятость.
- [ ] Формула роста/миграции на основе удовлетворённости/налогов/энергии.
- [ ] Оверлей Population (заселённость по клеткам).

### Этап E — Кредиты (1 день)
- [ ] Модель `Loan` и банк (простые правила выдачи).
- [ ] Модалка «Взять кредит» (сумма, срок, ставка), расчёт платежа.
- [ ] Списание процентов/платежей ежедневно, ограничения/блокировки при просрочке.
...
- [ ] Юнит-тесты формул экономики/энергетики/кредитов.
